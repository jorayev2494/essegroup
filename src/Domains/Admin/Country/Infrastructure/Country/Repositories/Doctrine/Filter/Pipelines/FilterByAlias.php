...
class FilterByAlias extends BaseFilterPipe
{
    ...
    {
        $queryBuilder->innerJoin(University::class, 'cu_a', Join::WITH, 'cu_a.countryUuid = c.uuid')
            ->innerJoin(Department::class, 'cud_a', Join::WITH, 'cud_a.universityUuid = cu_a.uuid')
            ->andWhere('cud_a.aliasUuid IN (:aliasUuids)')
