...
class FilterByLanguage extends BaseFilterPipe
{
    ...
    {
        $queryBuilder->innerJoin(University::class, 'cu_l', Join::WITH, 'cu_l.countryUuid = c.uuid')
            ->innerJoin(Department::class, 'cud_l', Join::WITH, 'cud_l.universityUuid = cu_l.uuid')
            ->andWhere('cud_l.languageUuid IN (:languageUuids)')
